...
async function loadMirrors() {
  for (const path of MIRROR_SOURCES) {
    try {
      ...
      if (r.ok) { state.mirrorsData = await r.json(); return; }
    } catch (_) {}
  }
...
function renderWorkflow() {
  ...
  byId("workflowSection").innerHTML = `<div class="rounded-xl border border-white/10 bg-white/5 p-3"><div class="grid gap-2 md:grid-cols-4">${[
...
function renderActions() {
  byId("actionsSection").innerHTML = `<div class="rounded-xl border border-white/10 bg-white/5 p-3"><div class="flex flex-wrap gap-2">
  ...
  byId("runBenchmark").onclick = benchmark;
  byId("downloadReport").onclick = downloadMirrorReport;
  byId("mirrorsFile").onchange = uploadMirrors;
  byId("dockerfileUpload").onchange = uploadDockerfile;
}
...
function renderCategoryOverview() {
  ...
  byId("categoryOverview").innerHTML = `<div class="rounded-xl border border-white/10 bg-white/5 p-2"><div class="flex flex-wrap gap-2">${html}</div></div>`;
...
function renderLatencyCompact() {
  if (!state.mirrorsData?.mirrors) { byId("latencySection").innerHTML = ""; return; }
  if (!hasMirrorResult() && state.benchmarkState !== "running") { byId("latencySection").innerHTML = `<div class="rounded-xl border border-white/10 bg-white/5 p-3 text-sm text-slate-300">No check has run yet. Start with mirror check.</div>`; return; }
  ...
  byId("latencySection").innerHTML = `<div class="grid gap-2 md:grid-cols-2">${cards}</div>`;
...
function renderDockerfileWorkspace() {
  ...
  const opt = byId("optimizeDockerfile"); if (opt) opt.onclick = optimizeDocker;
  byId("clearDockerfile").onclick = ()=>{state.dockerInput="";state.dockerOutput="";state.dockerReport=null;state.dockerfileState="empty";renderDockerfileWorkspace();};
  byId("downloadOptimized").onclick = ()=>downloadText("Dockerfile.optimized", state.dockerOutput);
  byId("downloadDockerReport").onclick = ()=>downloadText("dockerfile-report.json", JSON.stringify(state.dockerReport,null,2));
}
...
async function benchmark() {
  if (!state.mirrorsData?.mirrors || state.benchmarkState === "running") return;
...
function downloadMirrorReport(){ downloadText("mirror-report.json", JSON.stringify(state.reportData||{}, null, 2)); }
...
function render(){ renderWorkflow(); renderActions(); renderCategoryOverview(); renderLatencyCompact(); renderEnvPreview(); renderDockerfileWorkspace(); }

loadMirrors().then(() => { render(); setStatusMessage(getDisplayStatus()); });
